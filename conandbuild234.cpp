...
void dirviewer()
{
	cout << dirfiles[0] << endl;
	cout << dirfiles[1] << endl;
	cout << dirfiles[2] << endl;
	cout << dirfiles[3] << endl;
	cout << dirfiles[4] << endl;
}
...
void trigger_kernel_panic() {
    cerr << "Kernel panic! (0x0D0A32)" << endl;
    raise(SIGSEGV); // Causes a segmentation fault to simulate kernel panic
}

void maybe_panic() {
    std::srand(std::time(nullptr)); // Seed random generator
    int chance = std::rand() % 100; // 0 to 99
    if (chance < 5) { // 5% chance to trigger panic
...
int main()
{
    ...
    while (true)
    {
        ...
        cout << "13. Version" << el;
        cout << "14. Updater" << el;
        cout << "15. Directory" << el;
        cout << "16. Code Editor (C++17)" << el;
        cout << "Select: ";
        getline(cin, input);
